...
vtkStandardNewMacro(vtkOvalSoutce);

vtkOvalSoutce::vtkOvalSoutce()
{
    ...
    vtkInformationVector* outputVector)
{
    ...
    for (j = 0; j < numPts; j++)
    {
        for (i = 0; i < 3; i++)
        {
            r[i] = this->AxisX * px[i] * cos((double)j * theta) + this->AxisY * py[i] * sin((double)j * theta);
            x[i] = this->Center[i] + r[i];
            //r[i] = px[i] * cos((double)j * theta) + py[i] * sin((double)j * theta);
            //x[i] = this->Center[i] + this->AxisX * r[i];
...
void vtkOvalSoutce::PrintSelf(ostream& os, vtkIndent indent)
{
    ...
    os << indent << "AxisX: " << this->AxisX << "\n";
    os << indent << "AxisY: " << this->AxisY << "\n";
